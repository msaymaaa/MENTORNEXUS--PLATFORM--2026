      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-18">
          ...
          <div className="flex items-center space-x-3">
            ...
            {/* Persona Switcher Badge Button (Evaluation & Multi-role testing) */}
            <button
              id="quick-role-switcher-btn"
              onClick={openPersonaSwitcher}
              className="inline-flex items-center space-x-2 px-2.5 py-1.5 rounded-lg text-xs bg-[#12141F] hover:bg-[#181B28] text-[#F5F2EB] border border-[#262A3C] transition-colors cursor-pointer"
              title="Switch persona or register a new user"
            >
              <span className="hidden sm:inline text-xs text-[#7A766E] font-mono uppercase">Role:</span>
              {currentUser && getRoleBadge(currentUser.role)}
              <ChevronDown className="w-3 h-3 text-[#7A766E]" />
            </button>
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.slice(0, 5).map((n) => (
                        <div
                          ...
                          className={`p-3.5 hover:bg-[#161925] cursor-pointer transition-colors ${
                            !n.read ? 'bg-[#181C2C]' : ''
                          }`}
                        >
                          <div className="flex items-start justify-between">
                            <h4 className={`text-xs font-semibold ${!n.read ? 'text-[#F5F2EB]' : 'text-[#9E9A90]'}`}>
                              {n.title}
                            </h4>
                          ...
                          <p className="text-xs text-[#9E9A90] mt-1 line-clamp-2 leading-relaxed">{n.message}</p>
                        </div>
                      ))
                    )}
                  </div>
                ...
                >
                  ...
                  <div className="py-1 text-xs">
                    ...
                    <button
                      ...
                      className="w-full px-4 py-2 text-left text-[#9E9A90] hover:text-[#F5F2EB] hover:bg-[#161925] flex items-center space-x-2 cursor-pointer"
                    >
                      <Users className="w-3.5 h-3.5 text-[#7A766E]" />
                      <span>Switch Persona / Register</span>
                    </button>
